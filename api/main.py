...
@app.post("/api/run", tags=["web"], operation_id="run_query",
          ...
          description="Runs the agent on a preset case or on a protein pair given by two UniProt "
                      ...
                      "`live`, `cited` or `pending`, and `usage` (seconds and cost of this query). Every "
                      "user-facing sentence comes in both languages as `{zh, en}`, so a client can switch "
                      "language without re-running the query. `conclusion.findings` are points to watch computed by code from "
                      "the tool outputs (with their `basis`); `conclusion.flags` are the agent's own flags, each a "
                      "plain-language rewording by the model (one extra model call, numbers checked to match) plus "
                      "the raw wording (`raw`) for a technical-details view. Interface residues are sorted by sequence number. "
         ...
         responses={200: {"content": {"application/json": {"example": {
             ...
             "conclusion": {"headline": {"zh": "<answer>", "en": "<answer>"},
                            "points": [{"zh": "<...>", "en": "<...>"}],
                            "findings": [{"plain": {"zh": "<sentence computed from tool outputs>", "en": "<...>"},
                                          "basis": "<the numbers it was computed from>"}],
                            "flags": [{"plain": {"zh": "<model's plain rewording>", "en": "<...>"},
                                       "raw": "<the system's original wording>"}],
                            "caveat": {"zh": "<what this is not>", "en": "<what this is not>"},
                            "details": "<the system's explanation, original wording>"},
             "evidence": [{"item": {"zh": "<what>", "en": "<what>"}, "detail": {"zh": "<detail>", "en": "<detail>"},
                           "label": "<live | cited | pending>",
                           "source": {"zh": "<where from>", "en": "<where from>"}, "records": "<count>"}],
